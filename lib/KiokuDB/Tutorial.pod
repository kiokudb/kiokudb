...
object without needing to pre-define anything like tables.
...
L<KiokuDB> can be asked to collapse such objects B<intrinsically>, that is
...
This exact API is intentionally still under-defined. In the future it will be
...
grunt work. The L<BerkeleyDB::Manager> object will be instantiated using the
...
To look up the objects, we use the manual key lookup query:
...
Or even more simply, if you don't mind loading the whole result set into memory:
